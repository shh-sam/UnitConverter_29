...
3. **Safe Refactor** — smell 1개만 처리.
   - **입출력·E00x 계약 변경 금지** — CLI 메시지 · 변환값 · 오류 코드 유지
   - **OCP/SRP (ECB)** — entity/control/boundary 추출·분리
...
| 유형 | 예시 |
|------|------|
| Extract Method | 파싱 로직 → `src/boundary/` |
| Extract Class | Formatter 분리 → `src/boundary/` |
| Registry 확장 | if/elif → `registry.register()` in `src/entity/` |
...
- **변경 파일**: src/entity/converter.py, src/entity/unit_registry.py
