...
2. **파일 결정** — U-* → `tests/boundary/` · D-CNV/D-REG-* → `tests/entity/` · D-CFG-* → `tests/control/`
...
# UI Track (boundary)
python -m pytest tests/boundary/test_u_in_01.py::test_u_in_01_empty_input -v
```
...
- **변경 파일**: tests/entity/test_d_cnv_01.py
...
| 금지 | 이유 |
|------|------|
| **`src/**` · `UnitConverter.py` 수정** | RED는 실패 테스트만 |
| **Logic Track entity/control Mock** | 실제 함수 호출 |
